...
---@param source number
---@param cb table
PSRCore.Functions.CreateCallback("psr-multicharacter:server:GetCurrentPeds", function(source, cb)
  if GetInvokingResource() ~= "psr-core" then
    --Every CreateCallback is triggered by QBR-CORE
    print("mm.... someone is trying to trigger this event from outside the resource")
    return
  end
  local license = PSRCore.Functions.GetIdentifier(source, 'license')
  local plyChars = {}
...
RegisterNetEvent("psr-multicharacter:server:CreateNewCharacter", function(data)
  ...
  if PSRCore.Player.Login(src, false, newData) then
    PSRCore.ShowSuccess(GetCurrentResourceName(), GetPlayerName(src) .. ' has succesfully loaded!')
    PSRCore.Commands.Refresh(src)
...
---@param cb boolean
---@param cid string
PSRCore.Functions.CreateCallback("psr-multicharacter:server:deleteCurrentCharacter", function(source, cb, cid)
  if GetInvokingResource() ~= "psr-core" then
    --Every CreateCallback is triggered by QBR-CORE
    print("mm.... someone is trying to trigger this event from outside the resource")
    return
  ...
  local Res = MySQL.single.await("SELECT EXISTS(SELECT 1 FROM players WHERE license = ? AND citizenid = ?) AS EX",
    { License, cid })
  if Res.EX == 1 then
    Player.Functions.DeleteCharacter(source, cid)
    Wait(200)
    ...
    print(PSRCore.Functions.GetPlayer(source).PlayerData.citizenid .. " is trying to delete a character with the ID: " .. cid)
    cb(false)
  end
end)
...
---@param cb any
---@param cid string
PSRCore.Functions.CreateCallback("psr-multicharacter:server:setNewModel", function(source, cb, cid)
  local src <const> = source
...
RegisterNetEvent('psr-multicharacter:server:loadUserData', function(cData)
  local src = source
  if PSRCore.Player.Login(src, cData.citizenid) then
    print('^2[psr-core]^7 ' .. GetPlayerName(src) .. ' (Citizen ID: ' .. cData.citizenid ..
