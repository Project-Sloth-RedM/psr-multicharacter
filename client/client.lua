...
local function onModelErrorDetected(cid)
    PSRCore.Functions.TriggerCallback("psr-multicharacter:server:setNewModel", function(_) end, cid)
end
...
RegisterNetEvent("psr-multicharacter:client:openMulticharacter",function(newPlayer)
    if not IsNuiFocused() then
        ...
        PSRCore.Functions.TriggerCallback("psr-multicharacter:server:GetCurrentPeds", function(data, char)
...
RegisterNUICallback("spawnSelectedCharacter",function(data,cb)
    ...
    if cData.location == "lastlocation" then
        ...
        if #(Coords - old) < 70 or #(Coords-default) < 30 then
            cb(false)
            exports["psr-core"]:Notify(2,"ERROR","Please select another spawn point")
            return
        end
    end
...
RegisterNUICallback("deleteCurrentCharacter",function(data,cb)
    ...
    PSRCore.Functions.TriggerCallback("psr-multicharacter:server:deleteCurrentCharacter",function(result)
